...
function onEdit(td) {
    ...
    confirmBtn.class = 'btn btn-success btn-block';
...
function AddData(e) {
    ...
    var class = document.getElementsByClassName('fullname' + '-error');
    console.log(class);
    class[0].class = 'fullname-error error';
    class[0].class = 'fullname-error error';
    class[0].class = 'fullname-error error';
    listName.push({
        name,
    ...
    row.innerHTML = listArrayName.join("");
...
function render(list) {
    list.map(item => {
        listArrayName.push(`<td>${item.name}</td>
	<td>${item.phone}</td>
	<td>${item.address}</td>
	<td>
...
// document.getElementById("search").addEventListener('onchange',()=>{
// 	let value = document.getElementById('search').value;
// 	listName.filter(item)
// });